  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 36 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: index * 0.08 }} className="flex flex-col" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <div className="relative overflow-hidden rounded-sm bg-gray-100 mb-3" style={{ aspectRatio: "3/4" }}>
        ...
        {product.badge && <span className={`absolute top-3 left-3 text-[9px] tracking-[.18em] uppercase font-bold px-2.5 py-1 rounded-sm text-white ${product.badge === "Sale" ? "bg-[#8B8B3E]" : "bg-gray-900"}`}>{product.badge}</span>}
        <motion.button onClick={() => setWished(!wished)} className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white/90 flex items-center justify-center" animate={{ opacity: hovered ? 1 : 0, y: hovered ? 0 : -6 }} transition={{ duration: 0.25 }}>
          <svg width="14" height="14" fill={wished ? "#8B8B3E" : "none"} stroke={wished ? "#8B8B3E" : "currentColor"} strokeWidth="2" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
        </motion.button>
        <motion.button className="absolute bottom-0 left-0 right-0 bg-red-900 text-white text-[10px] tracking-[.2em] uppercase font-bold py-3 hover:bg-[#8B8B3E] transition-colors" animate={{ y: hovered ? 0 : "100%" }} transition={{ duration: 0.3, ease: [0.22,1,0.36,1] }}>Quick Add</motion.button>
      </div>
      <StarRating rating={product.rating} />
      <p className="text-[10px] tracking-[.15em] uppercase text-[#8B8B3E] mt-1 mb-0.5">{product.designer}</p>
      ...
      <div className="flex items-center gap-2">
        ...
        {product.oldPrice && <><span className="text-gray-400 line-through text-sm" style={{ fontFamily: "'Cormorant Garamond', serif" }}>₹{product.oldPrice.toLocaleString()}</span><span className="text-[10px] text-[#8B8B3E] font-bold">−{Math.round((1-product.price/product.oldPrice)*100)}%</span></>}
      </div>
    ...
    <section className="py-20 bg-[#F0F0EC]">
      <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
        <div ref={ref} className="flex items-end justify-between gap-6 mb-10 flex-wrap">
          <div>
            <motion.p initial={{ opacity:0, y:10 }} animate={inView ? {opacity:1,y:0} : {}} transition={{duration:.5}} className="text-[11px] tracking-[.25em] uppercase text-[#8B8B3E] font-semibold flex items-center gap-2 mb-3"><span className="w-6 h-[1.5px] bg-[#8B8B3E]"/>Handpicked for You<span className="w-6 h-[1.5px] bg-[#8B8B3E]"/></motion.p>
            <motion.h2 initial={{ opacity:0, y:20 }} animate={inView ? {opacity:1,y:0} : {}} transition={{duration:.65,delay:.08}} className="font-bold text-gray-900" style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(2rem,4vw,3rem)"}}>New <em className="italic text-[#8B8B3E]">Arrivals</em></motion.h2>
          </div>
          <a href="/collections/all" className="text-[11px] tracking-[.2em] uppercase font-bold text-gray-400 hover:text-[#8B8B3E] transition-colors flex items-center gap-2">View All <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg></a>
        </div>
        <div className="flex gap-2 flex-wrap mb-10">
          {FILTERS.map(f => <button key={f} onClick={() => setActiveFilter(f)} className="text-[11px] tracking-[.15em] uppercase font-semibold px-4 py-2 rounded-sm border transition-all" style={{ background: activeFilter===f ? "#8B8B3E" : "#fff", borderColor: activeFilter===f ? "#8B8B3E" : "#e5e7eb", color: activeFilter===f ? "#fff" : "#6b7280" }}>{f}</button>)}
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-5 sm:gap-6">
          {PRODUCTS.map((p, i) => <ProductCard key={p.id} product={p} index={i} />)}
        </div>
        <div className="text-center mt-12">
          <button className="border-[1.5px] border-gray-200 hover:border-[#8B8B3E] hover:text-[#8B8B3E] text-gray-600 text-[11px] tracking-[.2em] uppercase font-bold px-10 py-3.5 rounded-sm transition-all">Load More Products</button>
        </div>
      </div>
    </section>
  );
